...
const handleCreateProductCategory = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        ...
        mes: productCategory ? 'Create a product category successfully' : 'Create a product category failed'
    })
})
...
const handleGetAllProductCategory = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        success: productCategory ? true : false,
        productCategory: productCategory ? productCategory : 'Create a product category failed'
    })
})
...
const handleUpdateProductCategoryById = asyncHandler(async (req, res) => {
    ...
    const productCategory = await ProductCateGory.findByIdAndUpdate(pc_id, req.body, {new: true});
    return res.status(200).json({
        success: productCategory ? true : false,
        data: productCategory ? productCategory : 'Update a product category failed'
    })
})
...
const handleDeleteProductCategoryById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        success: productCategory ? true : false,
        mes: productCategory ? 'Delete a product category successfully' : 'Delete a product category failed'
    })
})
